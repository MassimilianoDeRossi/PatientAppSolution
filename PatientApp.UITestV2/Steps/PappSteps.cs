    ...
    public class PappSteps
    {
            ...
            switch (OptName)
            {
                ...
                case "Strut Adjust":
                    if(isAnonymous)
                    {
                        ...
                        app.Tap("BtnStrutAdjustSampleButton");
                    }
                    else
                    {
                ...
                case "Close":
                    ...
                    break;
                case "Close prescription popup":
                    Utils.OnAndroid(() =>
                    {
                        //From android 10 u can intercept Close pressing "ImgClose"
                        Utils.WaitAndTapElement("ImgClose");
                        //Utils.GoBackOnAndroid(); //On Android we press back instead of close becasue we cannot intercept close button
                    });
                    Utils.OnIOS(() =>
                    {
                        Utils.WaitAndTapElement("BtnClose");
                    ...
                    break;
                case "Cancel": //this works only if you want to go back from share popup
                    //Utils.GoBackOnAndroid(); //can't press back button due a permission issue
                    Utils.OnIOS(() =>
                    {
                        Utils.WaitAndTapElement("Close");
                    });
        ...
        public void WhenUserEnterWith(string field, string myvalue)
        {
            switch (field)
            {
                    ...
                    if (myvalue != "empty")
                    {
                        ...
                        else
                        {
                            ...
                            Utils.OnAndroid(() =>
                            {
                                ...
                                app.Tap("custom"); //Android 11 not select automatically the textbox
                            });
                            app.ClearText();
                    ...
                    if (myvalue != "empty")
                    {
                        ...
                        if (Utils.IsVisible(x => x.Class(Utils.GetClassName(PlatformClassName.TimePicker))))
                        {
                            Utils.OnIOS(() =>
                            {
                                ...
                                try
                                {
                                    app.WaitForElement("AM", "", TimeSpan.FromSeconds(5));
                                    app.WaitForElement("PM", "", TimeSpan.FromSeconds(5));
                                    //Time Picker with AM/PM column
                                    isAMPM = true;
                                }
                                catch (Exception)
                                {
                                    //Time Picker without AM/PM column
                                }
                                ...
                                Utils.Wait(2000);
                                app.Tap(x => x.Property("text").Like(isAMPM ? 
                                    (Int32.Parse(DailyTime[0]) > 12 ? (Int32.Parse(DailyTime[0]) - 12).ToString() : (Int32.Parse(DailyTime[0]) < 10 ? DailyTime[0].Substring(1) : DailyTime[0])) 
                                    : (Int32.Parse(DailyTime[0]) < 10 ? DailyTime[0].Substring(1) : DailyTime[0])));
                            ...
                            Utils.OnAndroid(() =>
                            {
                                ...
                                app.Query(x => x.Class(Utils.GetClassName(PlatformClassName.TimePicker)).Invoke("setHour", int.Parse(DailyTime[0])));
                                app.Query(x => x.Class(Utils.GetClassName(PlatformClassName.TimePicker)).Invoke("setMinute", int.Parse(DailyTime[1])));
                ...
                case "Mood":
                    ...
                    Utils.OnIOS(() =>
                    {
                        app.Tap(x => x.Class(Utils.GetClassName(PlatformClassName.Slider)).Child().Child().Index(2));//non appaiono più le faccine al primo colpo se non tappi sul pallino --> bug segnalato
                    });
                    for (int i = 0; i < MoodTypes.Length; i++)
                    {
                        if (MoodTypes[i] == myvalue)
                        {
                            Utils.OnIOS(() =>
                            {
                                ...
                                app.Tap(x => x.Class(Utils.GetClassName(PlatformClassName.Slider)).Child().Child().Index(2));
                            });
                            Utils.OnAndroid(() =>
                            {
                                //app.SetSliderValue(x => x.Class(Utils.GetClassName(PlatformClassName.Slider)), 124 * i); //this could be correct also for IOS
                                var slider = app.Query(x => x.Class(Utils.GetClassName(PlatformClassName.Slider))).First();
                                app.TapCoordinates(i*(slider.Rect.CenterX - slider.Rect.X)*2/(MoodTypes.Length-1) + slider.Rect.X, slider.Rect.CenterY);
                            });
                            i = 10;
                        }
                    }
        ...
        public void ThenTheShouldBe(string field, string myvalue)
        {
            ...
            switch (field)
            {
                ...
                case "set daily time":
                    ...
                    {
                        if (!Utils.isAndroid && (!Utils.GetText("TimePickerEntryDailyTime").Contains(myvalue))) //on Android PM and AM not exists
                        {
                    ...
                    {
                        var myvalue24 = Utils.Convert12HoursFormatoTo24HoursFormat_OnlyOnAndroid(myvalue);
                        var multipleSelector = new Func<AppQuery, AppQuery>[] { x => x.Text(myvalue), x => x.Text(myvalue24) };
                        var selectorVisible = Utils.WaitElements(multipleSelector, "time 12h or 24h format");
                        app.Tap(selectorVisible); //todo wait and tap?
                    }
                    break;
                case "Goal":
                case "Insights":
                ...
                case "FrameIdInfo":
                    ...
                    Utils.TakeScreenshot("Numero riquadri corretto");
                    ...
                    Utils.TakeScreenshot("Valori e immagini corretti");

                    ThenOptionIs("start adjustment", "shown");
                    ThenOptionIs("postpone adjustment", "shown");
                    Utils.TakeScreenshot("Start and Do it later button visualized");
        ...
        public void WhenUserProfileWith(string p0, Table table)
        {
            ...
            if (isNormal)
            {
                ...
                Utils.TakeScreenshot("Screen after inserting Nickname");
                WhenUserTapOnOption("Next");
                if (givenNormalTable.TimePinSite != "empty")
                {
                    WhenUserEnterWith("set daily time", givenNormalTable.TimePinSite);
                }
                Utils.TakeScreenshot("Screen after inserting Time in set daily time field");
                WhenUserTapOnOption("Next");
                ...
                Utils.TakeScreenshot("Screen after set up personal goal");
                WhenUserTapOnOption("Next");
                ...
                Utils.TakeScreenshot("Screen after set up insights messages");
        ...
        public void ThenAnonymousProfileSettingsAreSuccessfullySaved(Table table)
        {
            ...
            Utils.TakeScreenshot("Screen after checking nickname field correctness");
            WhenUserTapOnOption("Next");
            ThenTheShouldBe("set daily time", givenTable.Time);
            Utils.TakeScreenshot("Screen after checking Time field correctness");
        }
        ...
        public void ThenNormalProfileSettingAreSucccessfullySaved(Table table)
        {
            ...
            Utils.TakeScreenshot("Screen after checking Personal Goal fields correctness");
            WhenUserNavigateToPageFromPage("Personal Goal", "User Profile");
            ThenTheShouldBe("Insights", givenTable.InsightsStat);
            ThenTheShouldBe("set daily time", givenTable.TimeInsigth);
            Utils.TakeScreenshot("Screen after checking Insights fields correctness");
        }
        ...
        public void ThenOptionIs(string element, string status)
        {
            switch (element)
            {
                case "How do you feel today?":
                    ...
                    Utils.Wait(1000); //on ios doesn't work without it
                    if (Utils.IsVisible(x => x.Marked("LblTitle").Text(Utils.GetTranslatedResx("LblTitleAllMyDailyTask"))))
                    {
        ...
        public void WhenUserMoodWith(string Action, string MoodName)
        {
            switch (Action)
            {
                case "set and save":
                    ...
                    Utils.TakeScreenshot("Screen after tap on How are you today?");
                    WhenUserEnterWith("Mood", MoodName);
                    Utils.TakeScreenshot("Screen after selection of mood " + MoodName);
                    WhenUserTapOnOption("Confirm this mood");
                    Utils.TakeScreenshot("Screen after confirmation of mood " + MoodName);
                    WhenUserTapOnOption("<");
                    break;
                case "update and save":
                    ...
                    Utils.TakeScreenshot("Screen after tap on Today I feel");
                    WhenUserEnterWith("Mood", MoodName);
                    Utils.TakeScreenshot("Screen after selection of mood " + MoodName);
                    WhenUserTapOnOption("Confirm this mood");
                    Utils.TakeScreenshot("Screen after confirmation of mood " + MoodName);
                ...
                case "update and not save":
                    ...
                    Utils.TakeScreenshot("Screen after cancellation of mood " + MoodName);
                    WhenUserTapOnOption("<");
                    break;
                case "update":
                    WhenUserEnterWith("Mood", MoodName);
        ...
        public void ThenAppears(string p0)
        {
            switch (p0)
            {
                ...
                case "Alert icon":
                    ...
                    break;
                case "Alert message":
                    Utils.TakeScreenshot("AlertMessage");
                    app.Tap(x => x.Text("OK"));
                    break;
            ...
            switch (action)
            {
                case "complete":
                    ...
                    else
                    {
                        ...
                        Utils.TakeScreenshot("Tap on " + frame);
                        WhenUserTapOnOption("Start Adjustment");
                        Utils.TakeScreenshot("Start of adjustment");
                ...
                case "postpone":
                    ...
                    Utils.TakeScreenshot("Tap on Do it later");
        ...
        public void ThenPageVisualizedCorrectly(string strut)
        {
            ...
            */
            ...
            Utils.TakeScreenshot("Rotation images correct");
            ...
            Utils.TakeScreenshot("Background images correct");
            ...
            Utils.TakeScreenshot("Length and Click value correct");
        }
        ...
        public void ThenOptionPageOfWorkCorrectly(string what)
        {
            if (what.Contains("Strut"))
            {
                if (what == "Strut 1")
                {
                    ...
                    Utils.TakeScreenshot("Tappig next option next strut is visualized");
                ...
                if (what == "Strut 6")
                {
                    ...
                    Utils.TakeScreenshot("Tappig back option previous strut is visualized");
                    WhenUserTapOnOption("Next");
                }
            ...
            switch (which)
            {
                case "Pin site care":
                    ...
                    if (status == "to do")
                    {
                        ...
                        WhenUserEnterWith("set daily time", "11:30");
                        Utils.TakeScreenshot("Daily time setted.");
                        if (isAnonymous)
                        {
                            WhenUserTapOnOption("Done");
                        }
                    ...
                    if (status == "not to do")
                    {
                        ...
                        WhenUserEnterWith("set daily time", "11:59");
                        Utils.TakeScreenshot("Daily time setted.");
                        if (isAnonymous)
                        {
                            WhenUserTapOnOption("Done");
        ...
        public void checkDateTimeAndFrameIDNameOfAdjustment()
        {
            ...
            Utils.TakeScreenshot("Data e ora dell'adjustment corrette");

            if (Utils.GetText("LblFrameId").Split(' ').Last() != FrameIDTapped)
            {
                Assert.Fail("Il frame ID tappato è:" + FrameIDTapped);
            }
            Utils.TakeScreenshot("Nome del Frame ID associato corretto");
        }
        ...
        {
            FrameSeen = Convert.ToString(Utils.ExecuteBackdoor(Backdoors.GetFrameStrutsAdj));
            BadgeSeen = Convert.ToString(Utils.ExecuteBackdoor(Backdoors.GetBadgeStrutsAdj));
        }
    }
}
