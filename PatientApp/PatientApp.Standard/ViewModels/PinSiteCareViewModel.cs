...
namespace PatientApp.ViewModels
{
  ...
  public class PinSiteCareViewModel : BaseViewModel
  {
    ...
    /// </summary>
    private async Task RefreshDone()
    {
      var reminder = (await _dbService.GetRemindersAtDate(_sysUtility.Now, Reminder.ReminderType.PinSiteCareReminder)).FirstOrDefault();
    ...
    {
      var reminder = (await _dbService.GetRemindersAtDate(_sysUtility.Now, Reminder.ReminderType.PinSiteCareReminder)).FirstOrDefault();
      var expiredReminders = await _dbService.GetRemindersToDate(_sysUtility.Now, Reminder.ReminderType.PinSiteCareReminder);

      // Delete all expired reminders (we can have reminder not set in previously days)
      if (await _dbService.DeleteReminders(expiredReminders))
      {
