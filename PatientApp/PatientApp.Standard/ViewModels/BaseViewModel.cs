  ...
  public abstract class BaseViewModel : ObservableObject
  {
    ...
    protected void RebuildPinSiteCareCalendarAndReminders(TimeSpan pinSiteCareTime)
    {
      Task.Run(async () =>
      {
        ...
        await _dbService.RebuildPinSiteCareCalendar(IsLoggedIn, AppSettings.Instance.PinSiteCareEnabled, AppSettings.Instance.PinSiteCareTime,
    ...
    /// </summary>
    protected async Task RescheduleRemindersIfNeeded()
    {
      if (_isRescheduling)
        return;
            ...
            settings.SchedulingLastDateTime.Value.Date < _sysUtility.Now.Date))
      {
        ...
        if (deleted)
        {
          ...
          var allReminders = await _dbService.GetReminders();
          var reminders = allReminders.Where(r => !r.IsChecked && r.DateTime > _sysUtility.Now && r.DateTime <= endDateTime).OrderBy(r => r.DateTime);
    ...
    protected async Task UpdateSigningCertificate(IApiClient _apiClient)
    {
      try
      {
        ...
        if (result.Success)
        {
          if (result.Data.Exists)
          {
            ...
            var isValid = true;
#else
            var isValid = DependencyService.Get<ICryptoService>().CheckCertificate(result.Data.Certificate);
#endif
            if (isValid)
      ...
      {
        AppLoggerHelper.LogEvent("Unhandled GetSigningCertificate exception", ex != null ? ex.ToString() : "No details", TraceLevel.Error);
      }
      finally
      {
      }
