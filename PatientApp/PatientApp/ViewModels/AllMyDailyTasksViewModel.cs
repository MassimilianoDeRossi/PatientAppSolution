...
namespace PatientApp.ViewModels
{
    ...
    public class AllMyDailyTasksViewModel : BaseViewModel
    {
        ...
        private void ViewAppeared(BaseContentPage page)
        {
        ...
        /// </summary>
        private void InitOrRefreshView()
        {
            Date = _sysUtility.Now;
            ReloadTodoTaskList(Date);
            ReloadDoneTaskList(Date);
        ...
        private void ReloadTodoTaskList(DateTime when)
        {
            var todoActivities = _dbService.GetRemindersAtDate(when);
            TodoTaskList.Clear();
            ...
            var moodSet = _dbService.GetHistoryLogItemsAtDate(when, LogHistoryItem.ItemTypeEnum.MoodSelfAssessment).Any();
            if (!moodSet)
            {
            ...
            foreach (var curActivity in todoActivities.OrderBy(r => r.DateTime))
            {
                ...
                {
                    var frameid = _dbService.GetPrescritionById(curActivity.PrescriptionId).FrameID;
        ...
        /// </summary>
        /// <param name="when"></param>
        private void ReloadDoneTaskList(DateTime when)
        {
            ...
            var doneActivities = _dbService.GetHistoryLogItemsDoneActivitesAtDate(when);
            ...
            foreach (var curActivity in doneActivities.OrderBy(a => a.EventDateTime))
            {
                ...
                switch (curActivity.ItemType)
                {
                    ...
                    case LogHistoryItem.ItemTypeEnum.StrutAdjustmentDone:
                        var strut = _dbService.GetStrutAdjustmentById(curActivity.LocalEntityId.Value);
                        if (strut != null)
                        {
                            var frameid = _dbService.GetPrescritionById(strut.PrescriptionId).FrameID;
                            vdt.Description = string.Format("{0} {1}", Resources.PatientApp.AllMyDailyTask_StrutAdjDesc, frameid);
                        }
                        else
                        {
