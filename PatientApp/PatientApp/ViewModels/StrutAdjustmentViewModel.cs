    ...
    public class StrutAdjustmentViewModel : BaseViewModel
    {
        ...
        private void WizardNextCommandExecute()
        {
            ...
            else
            {
                ...
                if (_dbService.SaveStrutAdjustment(SelectedAdjustment))
                {
        ...
        {
            var prescriptions = _dbService.GetPrescriptions().OrderBy(p => p.FrameID);

            var recapList = new List<PrescriptionDetail>();

            foreach (var p in prescriptions)
            {
                var adjItems = _dbService.GetPrescriptionStrutAdjustments(p.Id, true).OrderBy(a => a.DateOfAdjustment);
        ...
        /// </summary>
        private void DeleteOlderThanTodayStruts()
        {
            if (_dbService.DeleteStrutAdjustmentsToDate(_sysUtility.Now.Date.AddDays(-1), true))
            {
